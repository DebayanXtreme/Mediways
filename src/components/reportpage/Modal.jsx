...
const Modal = ({ handleClose, show, fetchData, children }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const imageRef = ref(storage,`Report/${uniqueFileName}`);
      uploadBytes(imageRef,file).then(data=>{
        getDownloadURL(data.ref).then(val=>{
          setImgUrl(val);
          
        // setImgUrl(downloadURL);
        const valRef = collection(txtDB,'reportData');
  ...
  return (
    <div className={styles.modal} style={showStyle}>
      <section className={styles.modal_main}>
        {children}
        <h1 className={styles.formTitle1}>Report</h1>
        ...
        <div className={styles.inputDiv}>
          <div>
            ...
            <input
              ...
              value={name}
              placeholder="Organization Name/ Report Type"
